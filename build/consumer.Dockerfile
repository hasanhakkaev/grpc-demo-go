...
RUN CGO_ENABLED=0 go build -a -ldflags "-s -w -X main.Version=${CONSUMER_VERSION} -X main.BuildTime=${BUILD_TIME}" -o consumer cmd/consumer/main.go
...
ENV CONSUMER_VERSION=${CONSUMER_VERSION}
